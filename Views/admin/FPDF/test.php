...
require 'fpdf.php';
    ...
    $con = new PDO('mysql:host=localhost;dbname=make-up','root','');
        ...
        $livreur = $con->prepare($query);
         $livreur->execute();
         if($livreur->rowCount()!=0)
            {
                while($row = $livreur->fetch())
                     {
                                                 $pdf->Cell(15);
                         $pdf->Cell(40,10,$row['PersonID'],1,0,'C');
                         $pdf->Cell(40,10,$row['nom'],1,0,'C');
                         $pdf->Cell(40,10,$row['prenom'],1,0,'C');
                         $pdf->Cell(40,10,$row['age'],1,1,'C');
                     }
            }   
        $pdf->Output(); 
